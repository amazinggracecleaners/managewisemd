...
export function EmployeeView({
  ...
  const [editingNoteForSchedule, setEditingNoteForSchedule] =
    useState<CleaningSchedule | null>(null);
  const [currentNote, setCurrentNote] = useState("");

  const [isTimesheetDialogOpen, setIsTimesheetDialogOpen] =
    useState(false);
  ...
  const scheduleForDay = (date: Date) => {
    return schedules.filter((s) => {
      ...
      const isAssigned = s.assignedTo.includes(employee.name);
      if (!isAssigned) return false;

      const schStart = parseISO(s.startDate);
      if (date < startOfDay(schStart)) return false;

      if (s.repeatUntil && date > endOfDay(parseISO(s.repeatUntil)))
        return false;
      ...
      switch (s.repeatFrequency) {
        ...
        case "every-3-weeks": {
          ...
          if (
            s.repeatFrequency === "every-2-weeks" &&
            weekDiff % 2 === 0
          )
            return true;
          if (
            s.repeatFrequency === "every-3-weeks" &&
            weekDiff % 3 === 0
          )
            return true;
          return false;
  ...
  const offScheduleActiveShifts = useMemo(() => {
    ...
    const scheduledSiteNames = new Set(
      scheduleForDay(currentDate).map((s) => s.siteName)
    );

    return sessionsForEmployee.filter(
      (s) =>
        s.active &&
        s.in?.site &&
        !scheduledSiteNames.has(s.in.site)
    );
  }, [sessionsForEmployee, currentDate, schedules]);

  const weeklySchedule = useMemo(() => {
    const startOfUserWeek = startOfWeek(today, {
      weekStartsOn: settings.weekStartsOn,
    });
    const week = [];
    for (let i = 0; i < 7; i++) {
  ...
  const handleSaveNote = useCallback(() => {
    ...
    updateSchedule(editingNoteForSchedule.id, {
      note: currentNote,
    });
  ...
  const hasOpenShiftForSiteOnDate = useCallback(
    (siteName: string, forDate: Date) => {
      return sessionsForEmployee.some((s) => {
        ...
        // Treat the shift as belonging to the calendar day of the clock-in
        const inDay = startOfDay(new Date(s.in.ts));
        return isSameDay(inDay, startOfDay(forDate));
      });
    },
    [sessionsForEmployee]
  );
  ...
  const handleClockInOut = useCallback(
    (action: "in" | "out", siteName: string) => {
      ...
      if (action === "in") {
        ...
        // ✅ Past + future days behave like current day, but using currentDate as the timestamp
  ...
  const calculateHoursForPeriod = useCallback(
    (startDate: Date, endDate: Date) => {
      ...
      for (const s of sessionsForEmployee) {
        ...
        const sessionEnd = s.out?.ts ?? Date.now(); // active shift
  
        // overlap of [sessionStart, sessionEnd] with [from, to]
  ...
  const activeShiftsCount = useMemo(() => {
    ...
    for (const s of settings.sites) {
      if (isClockedIn(s.name, employee.id)) {
        activeSites.add(s.name.toLowerCase());
      }
    }
  ...
  const currentSiteStatuses = useMemo(
  ...
  const handleViewTimesheet = (
    ...
    const sessions = groupSessions(periodEntries);
    setTimesheetData({ period, sessions });
    setIsTimesheetDialogOpen(true);
  };

  // Hours-per-site for label when status is "complete"
  const getHoursForSiteDay = useCallback(
    (siteName: string, forDate: Date) => {
      ...
      // Use ALL entries for this employee + site, not just ones inside the day window
  ...
  return (
    <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Based on your completed shifts.
                </CardDescription>
              </CardHeader>
              <CardContent>
                  ...
                  <Tabs defaultValue="daily">
                    ...
                    <TabsContent value="daily">
                      ...
                      <ScrollArea className="h-72">
                        ...
                        (isCurrentDayToday && offScheduleActiveShifts.length >
                          0) ? (
                          <ul className="space-y-4">
                            {scheduleForDay(
                              ...
                              (schedule) => {
                                ...
                                const siteBonus =
                                  scheduleSite?.bonusType &&
                                  scheduleSite.bonusAmount
                                    ? scheduleSite.bonusType ===
                                      "hourly"
                                      ? ` (+$${scheduleSite.bonusAmount.toFixed(
                                          2
                                        )}/hr)`
                                      : ` (+$${scheduleSite.bonusAmount.toFixed(
                                          2
                                    ...
                                    <div className="flex gap-2 mt-2 flex-wrap">
                                      {clockedInAtThisSite ? (
                                        ...
                                        <Button
                                          ...
                                          disabled={
                                            isManagerPreview || clockInDisabled
                                          }
                                        >
                    ...
                    0 ? (
                      ...
                      <TableRow>
                        ...
                        >
                          {employee.name
                            ? "No entries yet."
                            : "Enter your name to see your activity."}
                        </TableCell>
                      </TableRow>
                    )}
                  </TableBody>
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              Edit Note for{" "}
              {
                editingNoteForSchedule
                  ?.siteName
              }
            </DialogTitle>
            <DialogDescription>
              This note is shared with
              your manager and other
              employees on this
              schedule.
            </DialogDescription>
          </DialogHeader>
      ...
      {/* Timesheet Dialog */}
      <Dialog
        open={isTimesheetDialogOpen}
        onOpenChange={setIsTimesheetDialogOpen}
      >
            ...
            <Table>
              ...
              <TableBody>
                {timesheetData.sessions
                  ...
                  timesheetData.sessions
                  .filter(
        (s): s is Session & { in: NonNullable<Session["in"]> } => !!s.in
      ).map(
                    (s) => (
                      <TableRow
                        key={s.in.id}
