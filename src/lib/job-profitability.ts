...
export type JobProfitRow = {
  ...
  revenue: number;
  labor: number;           // wage * hours accrued on this day
...
function isScheduleActiveOnDate(
  ...
  const monthDiff =
    (getYear(date) - getYear(schStart)) * 12 + (getMonth(date) - getMonth(schStart));
...
function resolveRevenueForSiteDay(
  siteName: string,
  isoDate: string,
  opts: {
    schedules: CleaningSchedule[];
  ...
  const inv = opts.invoices.find(
    (i) => i.siteName === siteName && (i.date ?? "").slice(0, 10) === isoDate
  );
  if (inv?.total != null) return Number(inv.total) || 0;

  const activeDate = new Date(isoDate + "T12:00:00");
  const sched = opts.schedules.find(
    (s) => s.siteName === siteName && isScheduleActiveOnDate(s, activeDate, opts.settings.weekStartsOn)
  );
...
export function computeJobProfitability(args: Args): Map<string, JobProfitRow> {
  ...
  // ✅ Group ALL entries, not just entries from this day
  // This preserves sessions that cross midnight or close on another day.
  const sessions = groupSessions([...entries].sort((a, b) => a.ts - b.ts));
  ...
  for (const site of Array.from(siteSet).filter(Boolean)) {
    // ✅ Accrued labor for this site ON THIS DAY
    let labor = 0;
    ...
    for (const s of sessions) {
      ...
      // Overlap of this session with the selected day
      ...
      const emp =
        employees.find((e) => e.id === (s as any).employeeId) ??
        employees.find((e) => e.name === s.employee);
