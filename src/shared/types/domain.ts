...
export type OtherExpense = {
  ...
  site?: string; // legacy free-text site name
  ...
  /*
   * True when ManageWiseMD creates the expense automatically.
   */
  isAutoGenerated?: boolean;

  /*
   * Identifies how the expense was created.
   */
  source?: "site-rs-fee" | "site-other-fee" | "manual";

  /*
   * Month for which the site expense applies.
   *
   * Format: YYYY-MM
   * Example: "2026-08"
   */
  ...
  /*
   * Unique key used to prevent duplicate monthly site fees.
   *
   * Examples:
   * `${siteId}_${expensePeriod}_rs-fee`
   * `${siteId}_${expensePeriod}_other-fee`
   */
  autoExpenseKey?: string | null;

  /*
   * Fee configuration used when the expense was generated.
   */
  feeType?: "percent" | "fixed";
  feeValue?: number;

  /*
   * Site revenue used to calculate a percentage-based fee.
   */
  revenueBasis?: number;
